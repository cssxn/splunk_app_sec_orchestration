...
require([
        ...
        'SA-ldapsearch/splunkjs-private-1.6.0' // hard coded with export name _splunkjs so no need for assignment
    ],
    function ($, _, modal, mvc, Messages, SearchManager, SimpleView) {
                ...
                if (messages.length == 0) {
                    ...
                    try {
                        messageText = messagePart[2]
                            .substring(1, messagePart[2].length)
                            .replace(/""/g, '"')
        ...
        var configurationEditor = {
            ...
            initialize: function (service) {
                ...
                self._service = new _splunkjs.Service(service.http, {
                ...
                var defaultConfigurationStanza = new _splunkjs.Service.Endpoint(self._service, "properties/ldap/default");
                ...
                _splunkjs.Async.parallel(
                    [
                        ...
                        function (done) { // Get default storage password
                            ...
                            defaultStoragePassword.fetch({}, function (err, response) {
                                ...
                                if (err == null) {
                                    return done(null, response.properties().clear_password);
                                }
                    ...
                    function (err, defaultSettings, defaultPassword, configurationStanzas) {
                        ...
                        self._transferSettings(defaultSettings, defaultPassword);
            ...
            read: function () {
                ...
                _splunkjs.Async.parallel(
                    [
                        ...
                        function (done) { // Read storage password
                            ...
                            storagePassword.fetch({}, function (err, resource) {
                                ...
                                if (err == null) {
                                    return done(null, {err: null, value: resource.properties().clear_password});
                                }
                    ...
                    function (err, settings, password) { // Transfer settings
                        ...
                        self._transferSettings(settings.value, password.value);
                        self._isDirty(false);
                        ...
                        [
                            function (done) { // Get default password for comparison
                                ...
                                storagePassword.fetch({}, function (err, resource) {
                                    ...
                                    var endpoint = new _splunkjs.Service.Endpoint(self._service,
            ...
            _transferSettings: function (settings, password) {
                ...
                else if (settings.password.length >= 4 && settings.password.slice(0, 4) == '{64}') {
                    settings.password = window.atob(settings.password.slice(4));
                }
                ...
                $('#alternatedomain,#basedn,#server,#port,#ssl,#binddn,#password').each(function (index, inputElement) {
                    ...
                    if (selector.is(':checkbox')) {
                        selector.prop('checked', value == selector.val());
                    }
                    else {
                        selector.val(value);
                    }
