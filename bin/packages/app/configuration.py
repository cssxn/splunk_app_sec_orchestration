# coding=utf-8
#
# Copyright © 2015 Splunk, Inc. All Rights Reserved.
...
class Configuration(object):
    Credentials = namedtuple(b'Credentials', [b'realm', b'username', b'password', b'authorization_id'])
    _tls = None
    ...
    def _ensure_unique_configuration_names(self, domain, alternatedomain):
        ...
        if existing_configuration:
            ...
            self.command.error_exit(ValueError('Domain {0} clashes with alternatedomain = {1} in [{2}].'.format(
        ...
        if alternatedomain:
            ...
            if existing_configuration:
                ...
                if alternatedomain == domain or alternatedomain == existing_domain:
                    self.command.error_exit(ValueError(
                        'Alternate domain {0} in [{1}] clashes with domain = [{2}].'.format(
                            alternatedomain, domain, existing_domain)))

                if alternatedomain == existing_alternatedomain:
                    self.command.error_exit(ValueError(
                        'Alternate domain {0} in [{1}] clashes with alternatedomain = {2} in [{3}]'.format(
    ...
    def _get_tls(self):
        ...
        configuration_file = command.service.confs['ssl']
        ...
        configuration_file = command.service.confs['server']
        ...
        for ssl_version in ssl_versions:
            ...
            if member not in ('*', 'ssl2', 'ssl3', 'tls', 'tls1.0', 'tls1.1', 'tls1.2'):
                ...
                command.error_exit(ValueError(message))
                return
    ...
    def _read_all_configurations(self):
        ...
        for configuration_stanza in self.command.service.confs['ldap'].iter():
    ...
    def _read_default_configuration(self):
        ...
        try:
            ...
            command.error_exit(error, 'The default configuration stanza for ldap.conf is missing.')
            return
    ...
    def _reset_fields(self, domain, settings):
        ...
        self.paged_size = self._get_value(settings, 'paged_size', default=1000, validate=Integer(1, 65535))

        for option in command.options.itervalues():  # override settings with command option values, if they're present
