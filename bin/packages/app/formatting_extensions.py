...
_attribute_syntaxes = {  # Attribute oMSyntax map
    ...
    b'1':   'Boolean',                    # 2.5.5.8        \x550508           TRUE or FALSE values.
    b'2':   'Integer',                    # 2.5.5.9        \x550509           A 32-bit number.
    b'4':   'String(Octet)',              # 2.5.5.10       \x55050A           A string of bytes (Octet).
    b'6':   'String(Object-Identifier)',  # 2.5.5.2        \x550502           The object identifier.
    b'10':  'Enumeration',                # 2.5.5.9        \x550509           A 32-bit enumeration.
    b'18':  'String(Numeric)',            # 2.5.5.6        \x550506           A sequence of digits.
    b'19':  'String(Printable)',          # 2.5.5.5        \x550505           Printable case-sensitive string.
    b'20':  'CaseIgnoreString(Teletex)',  # 2.5.5.4        \x550504           Teletex. Does not differentiate upper case
                                          #                                   and lowercase.
    b'22':  'String(IA5)',                # 2.5.5.5        \x550505           IA5-String. Character set is case-
                                          #                                   sensitive.
    b'23':  'String(UTC-Time)',           # 2.5.5.11       \x55050B           UTC Time.
    b'24':  'String(Generalized-Time)',   # 2.5.5.11       \x55050B           Generalized-Time.
    b'27':  'Case-Sensitive String',      # 2.5.5.3        \x550503           General String. Differentiates uppercase
                                          #                                   and lowercase.
    b'64':  'String(Unicode)',            # 2.5.5.12       \x55050C           Unicode string.
    b'65':  'LargeInteger',               # 2.5.5.16       \x550510           A 64-bit number.
    b'66':  'String(NT-Sec-Desc)',        # 2.5.5.15       \x55050F           A Microsoft® Windows NT® Security
                                          #                                   descriptor.
    b'127': 'Object'                      # 2.5.5.1        \x550501           The fully qualified name of an object in
                                          #                                   the directory.
        ...
        if ace_type == 0x12:  # SYSTEM_RESOURCE_ATTRIBUTE_ACE_TYPE
            ...
            def unpack_null_terminated_utf16_string(string, start):
                ...
                while struct.unpack_from(b'H', string, end) != 0:
                    pass
                return '"' + unicode(struct.unpack_from(str(end - start) + b's', string, start)[0], 'utf-16') + '"'

            def unpack_claim_security_attribute_type_int64(string, start):
                return unicode(struct.unpack_from(b'l', string, start)[0])
            ...
            def unpack_claim_security_attribute_type_octet_string(string, start):
                ...
                return b16encode(struct.unpack_from(str(octet_length) + b's', string, start))
            ...
            claim_value_offsets = struct.unpack_from(str(claim_count) + b'I', value, offset)
            ...
            if claim_type == 0x0001:    # CLAIM_SECURITY_ATTRIBUTE_TYPE_INT64
                ...
                claim_values = [unpack_claim_security_attribute_type_octet_string for x in claim_value_offsets]
                claim_type = 'TD'
            ...
            text += '(' + ';'.join(
                (
                    ...
                    account_sid,
                    '(' + ','.join(
                        itertools.chain((claim_name, claim_type, unicode(claim_flags)), claim_values)) + ')'
                )) + ')'
        else:
            text += '(' + ';'.join(
                (
...
def format_filetime(value):
    ...
    **References:**
    ...
    epoch_time = 0x019db1ded53e8000L
    seconds, hundreds_of_nanoseconds = divmod(file_time - epoch_time, 10000000L)
...
def format_sid(value, offset=0, rid_map=None):
        ...
        try:
            if rid == 0x00000000 and sub_authority_count == 1:
                ...
                if identifier_authority == 1:    # SECURITY_WORLD_SID_AUTHORITY
                    ...
                    text = '-'.join(('S', unicode(revision), unicode(identifier_authority), unicode(rid)))
            else:
                text = rid_map[rid]
            return text
        except KeyError:
    ...
    identifier_authority = unpack_identifier_authority()
    sub_authorities = struct.unpack_from(str(sub_authority_count) + b'I', value, offset + 8)

    text = '-'.join(itertools.chain(
        (
            'S',
            unicode(revision),
            unicode(identifier_authority) if identifier_authority < 0x100000000 else '{0:#014x}'.format(
                identifier_authority)
        ),
        itertools.imap(lambda p: unicode(p), sub_authorities)))
...
formatting_extensions = {
    ...
    b'1.2.840.113556.1.2.231':           format_attribute_syntax,       # oMSyntax

    b'1.2.840.113556.1.4.159':           format_filetime,               # Account-Expires
    b'1.2.840.113556.1.4.49':            format_filetime,               # Bad-Password-Time
    b'1.2.840.113556.1.4.13':            format_filetime,               # Builtin-Creation-Time
    b'1.2.840.113556.1.4.26':            format_filetime,               # Creation-Time
    b'1.2.840.113556.1.4.720':           format_filetime,               # dhcp-Update-Time
    b'1.2.840.113556.1.4.519':           format_filetime,               # Last-Backup-Restoration-Time
    b'1.2.840.113556.1.4.50':            format_filetime,               # Last-Content-Indexed
    b'1.2.840.113556.1.4.51':            format_filetime,               # Last-Logoff
    b'1.2.840.113556.1.4.52':            format_filetime,               # Last-Logon
    b'1.2.840.113556.1.4.1696':          format_filetime,               # Last-Logon-Timestamp
    b'1.2.840.113556.1.4.53':            format_filetime,               # Last-Set-Time
    b'1.2.840.113556.1.4.662':           format_filetime,               # Lockout-Time
    b'1.2.840.113556.1.4.66':            format_filetime,               # LSA-Creation-Time
    b'1.2.840.113556.1.4.2262':          format_filetime,               # ms-DS-Approximate-Last-Logon-Time-Stamp
    b'1.2.840.113556.1.4.1442':          format_filetime,               # ms-DS-Cached-Membership-Time-Stamp
    b'1.2.840.113556.1.4.1971':          format_filetime,               # ms-DS-Last-Failed-Interactive-Logon-Time
    b'1.2.840.113556.1.4.1970':          format_filetime,               # ms-DS-Last-Successful-Interactive-Logon-Time
    b'1.2.840.113556.1.4.1996':          format_filetime,               # ms-DS-User-Password-Expiry-Time-Computed
    b'1.2.840.113556.1.4.96':            format_filetime,               # Pwd-Last-Set
    b'1.2.840.113556.1.4.502':           format_filetime,               # Time-Vol-Change

    b'1.2.840.113556.1.4.750':           format_group_type,             # Group-Type

    b'1.2.840.113556.1.4.149':           format_guid,                   # Attribute-Security-GUID
    b'1.2.840.113556.1.4.533':           format_guid,                   # FRS-Replica-Set-GUID
    b'1.2.840.113556.1.4.43':            format_guid,                   # FRS-Version-GUID
    b'1.2.840.113556.1.4.1428':          format_guid,                   # ms-COM-ObjectId
    b'1.2.840.113556.1.4.2032':          format_guid,                   # ms-DFS-Generation-GUID-v2
    b'1.2.840.113556.1.4.2041':          format_guid,                   # ms-DFS-Link-Identity-GUID-v2
    b'1.2.840.113556.1.4.2033':          format_guid,                   # ms-DFS-Namespace-Identity-GUID-v2
    b'1.2.840.113556.1.6.13.3.18':       format_guid,                   # ms-DFSR-ContentSetGuid
    b'1.2.840.113556.1.6.13.3.23':       format_guid,                   # ms-DFSR-ReplicationGroupGuid
    b'1.2.840.113556.1.4.1949':          format_guid,                   # ms-DS-Az-Object-Guid
    b'1.2.840.113556.1.4.1360':          format_guid,                   # MS-DS-Consistency-Guid
    b'1.2.840.113556.1.4.2062':          format_guid,                   # ms-DS-Optional-Feature-GUID
    b'1.2.840.113556.1.4.1965':          format_guid,                   # ms-FVE-RecoveryGuid
    b'1.2.840.113556.1.4.1998':          format_guid,                   # ms-FVE-VolumeGuid
    b'1.2.840.113556.1.4.359':           format_guid,                   # Netboot-GUID
    b'1.2.840.113556.1.4.2':             format_guid,                   # Object-Guid
    b'1.2.840.113556.1.4.505':           format_guid,                   # OMT-Guid
    b'1.2.840.113556.1.4.333':           format_guid,                   # OMT-Indx-Guid
    b'1.2.840.113556.1.4.1224':          format_guid,                   # Parent-GUID
    b'1.2.840.113556.1.4.205':           format_guid,                   # PKT-Guid
    b'1.2.840.113556.1.4.148':           format_guid,                   # Schema-ID-GUID
    b'1.2.840.113556.1.4.122':           format_guid,                   # Service-Class-ID
    b'1.2.840.113556.1.4.362':           format_guid,                   # Site-GUID
    b'1.2.840.113556.1.4.336':           format_guid,                   # Vol-Table-GUID
    b'1.2.840.113556.1.4.334':           format_guid,                   # Vol-Table-Idx-GUID

    b'1.2.840.113556.1.2.1':             format_instance_type,          # Instance-Type

    b'1.2.840.113556.1.4.302':           format_sam_account_type,       # SAM-Account-Type

    b'1.2.840.113556.1.4.2154':          format_sid,                    # ms-Authz-Central-Access-Policy-ID
    b'1.2.840.113556.1.4.1410':          format_sid,                    # MS-DS-Creator-SID
    b'1.2.840.113556.1.4.1844':          format_sid,                    # ms-DS-Quota-Trustee
    b'1.2.840.113556.1.4.146':           format_sid,                    # Object-Sid
    b'1.2.840.113556.1.4.121':           format_sid,                    # Security-Identifier
    b'1.2.840.113556.1.4.609':           format_sid,                    # SID-History
    b'1.2.840.113556.1.4.667':           format_sid,                    # Sync-With-SID
    b'1.2.840.113556.1.4.1301':          format_sid,                    # Token-Groups
    b'1.2.840.113556.1.4.1418':          format_sid,                    # Token-Groups-Global-And-Universal
    b'1.2.840.113556.1.4.1303':          format_sid,                    # Token-Groups-No-GC-Acceptable

    b'1.2.840.113556.1.4.8':             format_user_flag_enum,         # User-Account-Control

    #  Type OIDs

    b'\'1.2.840.113556.1.4.907\'':       format_security_descriptor,    # LDAPTYPE_SECURITY_DESCRIPTOR
}
